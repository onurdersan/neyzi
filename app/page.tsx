...
const TurkishGrowthPercentileCalculator = () => {
  ...
  const getActiveAge = () => {
    ...
    if (childData.useCorrectedAge && childData.gestationalAge) {
      ...
      if (!isNaN(gestationalAgeWeeks)) {
        ...
        const finalCorrectedAge = correctedDecimalAge > 0 ? correctedDecimalAge : 0;
        return {
          chronological: chronologicalDecimalAge,
          corrected: finalCorrectedAge,
          display: finalCorrectedAge,
        };
      }
    }
    return {
  ...
  const calculatePercentiles = () => {
    ...
    if (isNaN(ageForCalculation) || !gender) {
      setResults(null);
      return;
    }
    ...
    const parsedHeadCircumference = parseFloat(headCircumference);

    if (!isNaN(parsedWeight) && lmsData.weight[gender]) {
      const closestAge = findClosestAge(ageForCalculation, lmsData.weight[gender]);
      const lms = lmsData.weight[gender][closestAge];
      if (lms) {
        const zScore = calculateZScore(parsedWeight, lms.L, lms.M, lms.S);
        const getCategory = (p: number, currentAge: number, z: number) => {
            if (currentAge < 2) {
                if (z <= -3) return { text: 'Ağır Malnutrisyon', color: 'text-red-600' };
                if (z <= -2) return { text: 'Orta Malnutrisyon', color: 'text-yellow-600' };
                if (z <= -1) return { text: 'Hafif Malnutrisyon', color: 'text-blue-600' };
            }
    ...
    if (!isNaN(parsedHeadCircumference) && lmsData.headCircumference[gender]) {
      ...
      if (lms) {
        const zScore = calculateZScore(parsedHeadCircumference, lms.L, lms.M, lms.S);
        const getCategory = (p: number) => {
    ...
    if (!isNaN(parsedWeight) && !isNaN(parsedHeight) && lmsData.bmi[gender]) {
  ...
  const copyResultsToClipboard = () => {
    ...
    if (childData.ageInputType === 'birthDate' && childData.birthDate) {
        ...
        if(!results.chronologicalAge) return;
                  ...
                  {childData.useCorrectedAge && (
                      <div>
                          ...
                          <select value={childData.gestationalAge} onChange={(e) => handleInputChange('gestationalAge', e.target.value)} className="w-full p-2 border border-gray-300 rounded-md">
                              ...
                              {Array.from({ length: 17 }, (_, i) => 24 + i).map(week => (
              ...
              {results && (results.weight || results.height || results.headCircumference || results.bmi) ? (
                <div className="space-y-4">
                  ...
                  {Object.entries(results).map(([key, data]: [string, any]) => {
                    if (typeof data !== 'object' || data === null || !data.value) return null;
                  ...
                  <div className="text-xs text-gray-500 pt-2">
                    <p>* Hesaplamalar {childData.useCorrectedAge && results.correctedAge ? `düzeltilmiş yaşa (${results.correctedAge} yıl)` : `kronolojik yaşa (${results.chronologicalAge} yıl)`} göre yapılmıştır.</p>
                  </div>
                  <div className="pt-4">
